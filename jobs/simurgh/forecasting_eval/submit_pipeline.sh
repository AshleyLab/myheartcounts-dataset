...
#
# Usage:
#   jobs/simurgh/forecasting_eval/submit_pipeline.sh            # full set + pipeline
...
submit() {
  ...
  jid=$(sbatch --parsable --exclude=simurgh2 \
        ...
        "$@" "jobs/simurgh/forecasting_eval/${script}")
  jid=${jid%%;*}
...
if [[ "${MHC_FORECAST_PIPELINE:-1}" == "1" && ${#ids[@]} -gt 0 ]]; then
  ...
  pj=$(sbatch --parsable --exclude=simurgh2 \
        --export=ALL,MHC_FORECAST_RUN_LABEL="${RUN_LABEL}" \
        --dependency="${DEP}" \
        jobs/simurgh/forecasting_eval/run_paper_pipeline.sbatch)
  pj=${pj%%;*}
