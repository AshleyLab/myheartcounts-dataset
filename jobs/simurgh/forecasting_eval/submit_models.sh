...
first="$(sbatch --parsable --export=ALL jobs/simurgh/forecasting_eval/run_dlinear.sbatch "$@")"
first="${first%%;*}"
submitted+=("${first}")
echo "Submitted run_dlinear.sbatch: ${first}"
for job in run_segrnn.sbatch run_mixlinear.sbatch; do
    out="$(sbatch --parsable --dependency="afterok:${first}" --export=ALL "jobs/simurgh/forecasting_eval/${job}" "$@")"
    job_id="${out%%;*}"
    submitted+=("${job_id}")
...
for job in run_chronos2.sbatch run_toto.sbatch; do
    out="$(sbatch --parsable --export=ALL "jobs/simurgh/forecasting_eval/${job}" "$@")"
    job_id="${out%%;*}"
    submitted+=("${job_id}")
...
if [[ "${MHC_FORECAST_AGGREGATE:-1}" == "1" ]]; then
    ...
    agg_out="$(sbatch --parsable \
        ...
        --export=ALL \
        jobs/simurgh/forecasting_eval/aggregate_results.sbatch)"
