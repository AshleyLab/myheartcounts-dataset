"""Evaluator wrapper for a trained PyPOTS DLinear checkpoint."""

from __future__ import annotations

from forecasting_evaluation.config import DLinearModelConfig, FeaturesConfig, ForecastingConfig
...
class DLinearModel(BasePyPOTSForecastingModel):
    """Evaluator adapter for PyPOTS DLinear."""

    def __init__(
        self,
        config: DLinearModelConfig,
        ...
        super().__init__(checkpoint_path=config.checkpoint_path, model_name="dlinear")

    def build_model(self):
        """Instantiate the PyPOTS DLinear model for inference."""
        ...
        moving_avg_window_size = self._get_training_config_value("model", "moving_avg_window_size")
        individual = self._get_training_config_value("model", "individual")
        ...
        return DLinear(
            ...
            moving_avg_window_size=(
                self.config.moving_avg_window_size
                if moving_avg_window_size is None
                else moving_avg_window_size
            ),
            individual=self.config.individual if individual is None else individual,
            d_model=d_model if d_model is not None else self.config.d_model,
    ...
    def n_steps(self) -> int:
        """Return the fixed history length expected by DLinear."""
        return (
    ...
    def n_pred_steps(self) -> int:
        """Return the forecast horizon expected by DLinear."""
        return (
            self._get_training_config_value("model", "n_pred_steps")
            or self.config.n_pred_steps
            or self.forecasting_config.forecasting_length
    ...
    def n_features(self) -> int:
        """Return the channel count expected by DLinear."""
        return (
            self._get_training_config_value("model", "n_features")
            or self.config.n_features
            or infer_n_features(self.features_config)
...
def _raise_missing_n_steps() -> int:
    raise ValueError(
        "Unable to infer evaluation history length for DLinear. "
        "Set model.dlinear.n_steps in the eval config or use a checkpoint that includes "
