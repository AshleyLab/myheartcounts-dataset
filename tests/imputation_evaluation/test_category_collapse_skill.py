...
class TestCollapsedReducer:
    """The ``_per_method_cell_collapsed_errors`` per-(user, category) reducer."""
    ...
    def test_workouts_drops_undefined_channels_per_user(self):
        """A user's workouts E is the nanmean over only their defined channels.
        ...
        workouts_idx = next(
            i for i, (n, _) in enumerate(BINARY_CATEGORIES_ORDERED) if n == "workouts"
        )
        np.testing.assert_allclose(E[0, workouts_idx], 0.3, rtol=1e-9)

    def test_user_with_zero_defined_channels_in_category_drops(self):
        """A user with NaN AUC across all workouts channels drops from the macro.

        Such a user contributes NaN and is dropped from the per-task macro.
        """
        n_users = 3
        per_user_auc = np.full((n_users, N_CHANNELS), np.nan, dtype=np.float64)
...
class TestCollapsedSkillScopes:
    """``compute_skill_scores`` scope partitioning and ``n_tasks`` counts."""

    def test_overall_uses_buckets_not_per_channel(self):
        """The ``overall`` scope sources binary categories from collapsed rows.

        It does NOT consume per-channel ``ch_7..ch_18`` binary rows.
        ``n_tasks`` counts scenarios present (= 1 here).
        """
                ...
                rows.append(
                    {
                        ...
                        if ch.startswith("ch_") and int(ch[3:]) < 7
                        else "binary",
                        "E": 0.5,
                    }
                )
        ...
        overall_3level = result[(result["method"] == "A") & (result["scope"] == "overall")]
    ...
    def test_no_collapsed_rows_omits_binary_cat_scopes(self):
        """Without ``cat_collapsed:*`` rows, the binary cat scopes don't appear.

        The binary ``cat:sleep`` / ``cat:workouts`` scopes are omitted, but
        the 3-level ``overall`` scope still emits, fed by the continuous
        buckets only.
        """
        rows = [
            {
        ...
        result = compute_skill_scores(
            _build_errors_df(rows),
            _build_errors_df(bl_rows),
            mode="pooled",
        )
...
class TestCollapsedChannelHelper:
    """The ``is_collapsed_channel`` channel-key predicate."""

    def test_is_collapsed_channel(self):
        """Only ``cat_collapsed:*`` keys are collapsed; ``ch_*`` keys are not."""
...
class TestOverallTwoStage:
    ...
    def _build_balanced_fixture(self):
        """Build a two-method fixture with all 4 buckets populated.

        Two scenarios on the continuous side, so activity / physiology have
        multiple tasks.
        """
        ...
        for sc in scenarios:
            ...
            for c in range(5):
                rows.append(
                    dict(
                        method="A", scenario=sc, channel=f"ch_{c}", channel_type="continuous", E=0.4
                    )
                )
            ...
            for c in (5, 6):
                rows.append(
                    dict(
                        method="A", scenario=sc, channel=f"ch_{c}", channel_type="continuous", E=0.5
                    )
                )
        ...
        for sc in scenarios:
            for c in range(7):
                rows.append(
                    dict(
                        method="B", scenario=sc, channel=f"ch_{c}", channel_type="continuous", E=0.5
                    )
                )
    ...
    def test_n_tasks_equals_number_of_scenarios(self):
        """n_tasks counts scenarios present at L1, not constituent tasks/buckets.

        The fixture has 2 scenarios (random_noise + temporal_slice), each
        populating all 4 buckets, so n_tasks == 2.
        """
        errors, bl = self._build_balanced_fixture()
        result = compute_skill_scores(errors, bl, mode="pooled")
        for method in ("A", "B"):
            row = result[(result["method"] == method) & (result["scope"] == "overall")]
            assert len(row) == 1
    ...
    def test_skill_equals_mean_of_bucket_log_ratios(self):
        ...
        got = float(
            result[(result["method"] == "A") & (result["scope"] == "overall")]["skill_score"].iloc[
                0
            ]
        )
    ...
    def test_per_channel_binary_scope_not_consumed(self):
        """Per-channel binary rows must NOT change ``overall``.

        Adding ``cat:sleep`` / ``cat:workouts`` per-channel rows leaves
        ``overall`` unchanged — the bucket priority rule is
        "collapsed wins for sleep / workouts".
        """
        errors, bl = self._build_balanced_fixture()
        baseline = float(
            compute_skill_scores(errors, bl, mode="pooled")
            .pipe(lambda d: d[(d["method"] == "A") & (d["scope"] == "overall")])["skill_score"]
            .iloc[0]
        )
        ...
        for sc in ("random_noise", "temporal_slice"):
            for c in (7, 8):  # sleep per-channel
                extra.append(
                    dict(method="A", scenario=sc, channel=f"ch_{c}", channel_type="binary", E=99.0)
                )
        ...
        for sc in ("random_noise", "temporal_slice"):
            for c in (7, 8, *range(9, 19)):
                bl_extra.append(
                    dict(
                        method="LOCF", scenario=sc, channel=f"ch_{c}", channel_type="binary", E=1.0
                    )
                )
...
class TestThreeLevelB2Form:
    """Per-scenario / semantic / overall all use the universal 3-level B.2 form.

    The operator is L3 within-bucket × scenario task mean → L2 mean over
    buckets within scenario → L1 log-space mean over scenarios in scope.
    Each scope tested below pins one closed-form aspect of that operator.
    """

    def _build_3level_fixture(self):
        """Build a single-method fixture with constant per-task R per bucket.

        Constant per-bucket R makes the bucket geomean and the per-scenario
        L2 mean closed-form.
    ...
    def test_per_scenario_4bucket_structural_skill(self):
        """Structural scenarios with all 4 buckets give the 4-bucket B.2 skill.
    ...
    def test_sleep_gap_two_bucket_skill(self):
        """sleep_gap has activity + physiology only → 2-bucket B.2 skill.

        No sleep/workouts buckets because binary channels aren't masked in
        sleep_gap. S = 1 − exp((log 0.4 + log 0.5) / 2).
        """
        errors, bl = self._build_3level_fixture()
        result = compute_skill_scores(errors, bl, mode="pooled")
    ...
    def test_workout_gap_and_intensity_failure_single_bucket_unchanged(self):
        """Single-bucket scenarios degenerate B.2 to the bucket value.
    ...
    def test_semantic_log_space_mean_over_3_scenarios(self):
        """Semantic is the L1 log-space mean over the 3 semantic scenarios.
    ...
    def test_overall_log_space_mean_over_6_scenarios(self):
        ...
        errors, bl = self._build_3level_fixture()
        result = compute_skill_scores(errors, bl, mode="pooled")
        log_struct = (np.log(0.4) + np.log(0.5) + np.log(0.6) + np.log(0.7)) / 4
        log_sg = (np.log(0.4) + np.log(0.5)) / 2
        log_wg = np.log(0.5)
        log_if = np.log(0.5)
        expected = 1.0 - np.exp((3 * log_struct + log_sg + log_wg + log_if) / 6)
        row = result[(result["method"] == "A") & (result["scope"] == "overall")]
        assert len(row) == 1
        np.testing.assert_allclose(
            float(row["skill_score"].iloc[0]),
            expected,
...
class TestTaskScopeWithCollapsedRows:
    """Task-grain ``task:*`` rows coexist with the Part D collapsed scopes.

    Every per-channel input AND every ``cat_collapsed:*`` input gets its own
    leaf scope, without polluting the aggregated scopes (``overall``,
    ``cat:*``).
    """
    ...
    def test_collapsed_rows_emit_task_scopes(self):
        ...
        bl_rows = [
            {
                ...
                "channel": ch,
                "channel_type": (
                    "binary_collapsed" if ch.startswith("cat_collapsed:") else "continuous"
                ),
                "E": 1.0,
            }
            for sc in scenarios
        ...
        skill_by_scope = result[result["method"] == "A"].set_index("scope")["skill_score"].to_dict()
        np.testing.assert_allclose(
            skill_by_scope["task:random_noise:cat_collapsed:sleep"],
            0.5,
            rtol=1e-9,
        )
        np.testing.assert_allclose(
            skill_by_scope["task:random_noise:cat_collapsed:workouts"],
            0.4,
            rtol=1e-9,
        )
        np.testing.assert_allclose(
            skill_by_scope["task:random_noise:ch_0"],
            0.6,
            rtol=1e-9,
        )

    def test_task_scope_count_per_scenario(self):
        """Exactly one ``task:*`` row per (method, scenario, channel) input row.

        The count is exact, not a lower bound.
        """
        rows = [
            {
                "method": "A",
