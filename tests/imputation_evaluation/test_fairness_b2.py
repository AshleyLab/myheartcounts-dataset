...
class TestBucketClassifier:
    ...
    def test_per_channel_binary_maps_to_none(self):
        """Per-channel binary rows (ch_7..ch_18) are NOT consumed: the
        sleep / workouts buckets are populated from cat_collapsed:* only.
        """
...
def _make_fairness_fixture():
    """Two methods × two subgroups × every bucket. Designed so the per-task
    log_ratios per bucket are constant inside each bucket (so the bucket
    geomean is closed-form) and the four bucket-log-ratios are deliberately
    different (so cross-bucket averaging is visible)."""
    rows = []
    ...
    def emit(method: str, targets: dict[str, float]):
        ...
        for c in range(5):
            for sg in ("f", "m"):
                rows.append(dict(
                    method=method, scenario="rn",
                    channel=f"ch_{c}", channel_type="continuous",
                    subgroup_attr="sex", subgroup_value=sg,
        ...
        for sg in ("f", "m"):
            rows.append(dict(
                method=method, scenario="rn",
                channel="cat_collapsed:sleep", channel_type="binary_collapsed",
                subgroup_attr="sex", subgroup_value=sg,
        ...
        for sg in ("f", "m"):
            rows.append(dict(
                method=method, scenario="rn",
                channel="cat_collapsed:workouts", channel_type="binary_collapsed",
                subgroup_attr="sex", subgroup_value=sg,
    ...
    for c in range(5):
        for sg, eb in baseline_pair().items():
            rows.append(dict(method="LOCF", scenario="rn",
                             channel=f"ch_{c}", channel_type="continuous",
                             subgroup_attr="sex", subgroup_value=sg, E=eb))
    for c in (5, 6):
        for sg, eb in baseline_pair().items():
            rows.append(dict(method="LOCF", scenario="rn",
                             channel=f"ch_{c}", channel_type="continuous",
...
class TestPerAttributeSkillTwoStage:
    ...
    def test_S_attr_equals_mean_of_bucket_log_ratios(self):
        """Closed-form check: each bucket's per-task log ratios are
        identical inside the bucket (all 5 activity channels share the
        same target=0.5), so the bucket-level log ratio is just
        log(targets[bucket]). The per-attribute skill is then
        1 - exp(mean of those 4 log values).
        """
        df = _make_fairness_fixture()
        out = _per_attribute_skill_keyed(
            df, extra_keys=[], baseline_method="LOCF",
        ...
        log_ratios = np.array([
            np.log(0.5), np.log(0.25), np.log(0.125), np.log(0.0625)
        ])
    ...
    def test_per_channel_binary_rows_ignored(self):
        """Sneaking catastrophic ch_7..ch_18 rows in must NOT change the
        per-attribute skill (the bucket classifier maps them to None).
        """
        df = _make_fairness_fixture()
        ...
        for ch_idx in range(7, 19):
            for sg in ("f", "m"):
                ...
                extra.append(dict(method="A", scenario="rn",
                                  channel=f"ch_{ch_idx}", channel_type="binary",
                                  subgroup_attr="sex", subgroup_value=sg, E=e_m))
                extra.append(dict(method="LOCF", scenario="rn",
                                  channel=f"ch_{ch_idx}", channel_type="binary",
                                  subgroup_attr="sex", subgroup_value=sg, E=e_b))
        ...
        with_leak = float(
            _per_attribute_skill_keyed(
                df_aug, extra_keys=[], baseline_method="LOCF",
                clip_lower=1e-2, clip_upper=100.0,
            )[lambda d: d["method"] == "A"]["S_attr"].iloc[0]
        )
...
class TestComputeFairSkillScoresEndToEnd:
    """End-to-end check on ``compute_fair_skill_scores`` (the deterministic
    point-flow entry that ``compute_imputation_paper_metrics`` calls)."""
    ...
    def test_overall_macro_averages_attributes(self):
        ...
        df_age["subgroup_value"] = df_age["subgroup_value"].map(
            {"f": "30-39", "m": "40-49"}
        )
        big = pd.concat([df, df_age], ignore_index=True)
        out = compute_fair_skill_scores(
            big, attrs=["sex", "age_group"], baseline_method="LOCF",
        )
        ...
        np.testing.assert_allclose(
            ...
            (float(a_row["fair_skill_score"].iloc[0])
             + float(b_row["fair_skill_score"].iloc[0])) / 2,
            rtol=1e-9,
        )
